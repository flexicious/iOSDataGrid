...
@implementation FLXSCsvExporter
...
-(NSString*)buildHeader:(FLXSFlexDataGrid*)grid
{
	...
	for(FLXSFlexDataGridColumn* col in grid.exportableColumns)
	{
        ...
        str = [[str stringByAppendingString:[FLXSCsvExporter escapeCsv:([FLXSExporter getColumnHeader:col colIndex:(colIndex++)])]] stringByAppendingString:@","];
	}
	//if([str indexOf:(@"ID")]==0)
...
- (NSString *)writeRecord:(FLXSFlexDataGrid *)grid record:(NSObject *)record {
	...
	for(FLXSFlexDataGridColumn* col in grid.exportableColumns)
	{
        ...
        str = [str stringByAppendingString:[self getSpaces:col spChar:@" "]];
         str =[ [str stringByAppendingString:[FLXSCsvExporter escapeCsv:([col itemToLabel:record:nil])]] stringByAppendingString:@"\","];
	}
	if(str.length>0){
